...
__all__ = ["GoogleModel", "GoogleModelConfig"]

JsonObject: TypeAlias = dict[str, Any]

_GOOGLE_VENDOR = "google"
...
class GoogleModelConfig(BaseModel):
    """Configuration for the Google Gemini answer model adapter."""

    model_config = ConfigDict(frozen=True)

    model_id: str = Field(min_length=1)
    api_key_env_var: str = Field(default="GOOGLE_API_KEY", min_length=1)
    timeout_seconds: float = Field(default=60.0, gt=0)
    max_retries: int = Field(default=3, ge=0)


@register_model("google")
class GoogleModel(AnswerModel):
    """Google Gemini API-backed answer model."""
    ...
    def __init__(
        ...
        *,
        ...
        self._client_factory = (
            client_factory if client_factory is not None else _create_google_client
        )
        ...
        self._budget = budget
        self._client: _GoogleClient | None = None
        self._pricing = load_vendor_pricing(_GOOGLE_VENDOR)
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            raise ModelError("Failed to create Google client.") from exc
    ...
    def _classify(self, exc: Exception) -> DispatchOutcome:
        ...
        if _is_timeout_error(exc):
            ...
            return DispatchOutcome(
                ...
                lambda _n: ModelTimeoutError("Google model request timed out.", model_id=model_id),
        ...
        if _is_authentication_error(exc):
            return DispatchOutcome(
                ...
                lambda _n: ModelAuthenticationError(
                    "Google authentication failed.", model_id=model_id
                ),
            )
        if _is_billing_error(exc):
            ...
            return DispatchOutcome(
                ...
                lambda _n: ModelBillingError(
                    "Google billing or quota is unavailable.", model_id=model_id, reason=reason
                ),
            )
        ...
        if kind == "rate_limit":
            return DispatchOutcome(
                ...
                lambda n: ModelRateLimitError(
                    "Google rate limit persisted after retries.", model_id=model_id, attempts=n
                ),
        ...
        if kind is not None:
            return DispatchOutcome(
                RetryDisposition.RETRYABLE,
                lambda n: ModelError("Google model request failed.", model_id=model_id, attempts=n),
            )
        return DispatchOutcome(
            RetryDisposition.TERMINAL,
            lambda n: ModelError("Google model request failed.", model_id=model_id, attempts=n),
    ...
    def _require_client(self) -> _GoogleClient:
        if self._client is None:
            raise ModelError("GoogleModel is not initialized.")
        return self._client
    ...
    def _result_from_response(self, response: object, *, latency_ms: float) -> APICallResult:
        ...
        try:
            ...
            raise ModelError(
                "Google pricing is not configured for the requested model.",
                model_id=self._config.model_id,
            ) from exc
...
def _create_google_client(
    *,
    ...
    return cast(
        _GoogleClient,
        genai.Client(api_key=api_key, http_options={"timeout": timeout_ms}),
    )
...
def _extract_output_text(response: object) -> str:
    ...
    if isinstance(text, str):
        return text
    raise ModelError("Google response did not include output text.")
...
def _extract_token_count(response: object, names: tuple[str, ...]) -> int:
    ...
    for name in names:
        ...
        if isinstance(count, int) and count >= 0:
            return count
    raise ModelError("Google response did not include token usage.")


def _compact_raw_response(response: object, *, output: str) -> JsonObject:
    raw: JsonObject = {"output_text": output}
    for name in ("model_version", "modelVersion", "response_id", "responseId"):
